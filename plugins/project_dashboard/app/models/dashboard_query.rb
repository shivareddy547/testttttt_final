...
class DashboardQuery < ActiveRecord::Base
  ...
  attr_accessible :project_id, :user_id,:column_names
  ...
  attr_protected :project_id, :user_id,:column_names
  ...
  def queried_table_name
    @queried_table_name ||= Issue.table_name
  end
  ...
  def sql_for_custom_field(field, operator, value, custom_field_id)
    ...
    if field =~ /^(.+)\.cf_/
      ...
      p "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++`reflect_on_association++++++++++++++++++++++"
      p queried_class=Issue.last
      p "+++++++++++++++++++++++end ++++++++++++++++++++++++++++++++++++="
  ...
  #   self
  # end
  def self.project_user_filter_init(user_id,project_id)
    dashboard_query = self.find_or_initialize_by_user_id_and_project_id(:user_id=>user_id,:project_id=>project_id)
    dashboard_query.user_id=user_id
    dashboard_query.project_id= project_id
